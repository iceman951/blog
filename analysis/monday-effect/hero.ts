...
out.push(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${W} ${H}" width="${W}" height="${H}" role="img" ` +
	`aria-label="For eight large US technology and growth stocks, the spread in a nine-year weekly dollar-cost-averaging outcome from choosing a weekday, against the spread from starting the same plan up to 51 weeks later. In this sample the start week moved the outcome far more than the weekday did.">`);
out.push(`<rect width="${W}" height="${H}" fill="${PAPER}"/>`);
out.push(`<text x="60" y="56" font-family="${F}" font-size="25" font-weight="700" fill="${INK}">Choosing the weekday barely moves a nine-year DCA</text>`);
out.push(`<text x="60" y="82" font-family="${F}" font-size="14" fill="${MUTED}">Best minus worst outcome, in percentage points of total return. $100 weekly for ${r.dca.byTicker[tickers[0]].fixedHorizon.installments} weeks; same installments and dollars in every plan.</text>`);

// legend
out.push(`<rect x="60" y="96" width="11" height="11" fill="${A}"/><text x="77" y="106" font-family="${F}" font-size="12" fill="${INK}">which weekday you buy on (median over 52 starts)</text>`);
out.push(`<rect x="400" y="96" width="11" height="11" fill="${B}"/><text x="417" y="106" font-family="${F}" font-size="12" fill="${INK}">which of 52 weeks you started in (Wednesday plan)</text>`);
...
tickers.forEach((t, i) => {
	const dd = r.dca.byTicker[t].fixedHorizon;
	const d = { weekdaySpreadPoints: dd.weekdaySpreadPoints.median, startWeekSpreadPoints: dd.startSpreadPoints.Wed, ratio: dd.startSpreadOverWeekdaySpread };
	...
	out.push(`<text x="${(x(d.startWeekSpreadPoints) + 10).toFixed(1)}" y="${(mid + 5).toFixed(1)}" font-family="${F}" font-size="12" fill="${MUTED}">${Math.round(d.ratio)}×</text>`);
